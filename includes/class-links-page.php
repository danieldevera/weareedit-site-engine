...
class EDIT_Links_Page {
	...
	public static function defaults() {
		...
		return [
			...
			'eb_headline'    => 'Early Bird · -10% de desconto',
			'eb_url'         => 'https://weareedit.io/formacao/',
			'eb_deadline'    => '2026-07-31T23:59:59',
	...
	public static function render() {
		...
		// Active campaign links (the ones promoted on social with "link na bio").
		// Update this list as campaigns start/end. Colours follow the format:
		// bootcamp = pink, workshop = teal, curso = blue.
		$campaigns = [
			[ 'label' => 'Bootcamp Advanced AI',      'sub' => 'Começa 29 Jun · 40h · Remote', 'url' => $home . '/formacao/bootcamp-advanced-artificial-intelligence/?utm_source=instagram&utm_medium=bio&utm_campaign=aai_bootcamp',     'bg' => '#f92869', 'tx' => '#ffffff' ],
			[ 'label' => 'Bootcamp Webflow 2.0',      'sub' => 'Começa 6 Jul · 32h · Remote',  'url' => $home . '/formacao/bootcamp-webflow-2-0-cria-sites-completos-ia/?utm_source=instagram&utm_medium=bio&utm_campaign=webflow_bootcamp', 'bg' => '#f92869', 'tx' => '#ffffff' ],
			[ 'label' => 'Workshop Ethics by Design', 'sub' => '16h · 100% online',            'url' => $home . '/formacao/workshop-etica-em-ia-online-2/?utm_source=instagram&utm_medium=bio&utm_campaign=ethics_workshop',  'bg' => '#60c5b3', 'tx' => '#0a0a0a' ],
		];
...
.hero::before{content:'';position:absolute;inset:-55%;z-index:0;pointer-events:none;will-change:transform;background:
...
@keyframes meshDrift{
 0%{transform:translate3d(-14%,-10%,0) scale(1.15) rotate(0deg);}
 50%{transform:translate3d(14%,10%,0) scale(1.45) rotate(8deg);}
 100%{transform:translate3d(-10%,14%,0) scale(1.25) rotate(-6deg);}
}
...
<main class="wrap">
  ...
  <section class="sec">
    <p class="label">Inscrições a decorrer</p>
    <?php foreach ( $campaigns as $c ) : ?>
    <a class="area" style="background:<?php echo esc_attr( $c['bg'] ); ?>;color:<?php echo esc_attr( $c['tx'] ); ?>;" href="<?php echo esc_url( $c['url'] ); ?>" data-evt="Campanha: <?php echo esc_attr( $c['label'] ); ?>" data-section="campanhas">
      <span class="nm"><b><?php echo esc_html( $c['label'] ); ?></b><span><?php echo esc_html( $c['sub'] ); ?></span></span>
      <span class="arr">&rarr;</span>
    </a>
    <?php endforeach; ?>
  </section>

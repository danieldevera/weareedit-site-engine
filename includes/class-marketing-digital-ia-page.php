...
class EDIT_Marketing_Digital_IA_Page {
    ...
    public static function ensure_page_exists() {
        ...
        if ( is_wp_error( $page_id ) ) {
            error_log( 'EDIT_Marketing_Digital_IA_Page: page creation failed — ' . $page_id->get_error_message() );
            return 0;
        }
    ...
    private static function set_rank_math_meta( int $page_id ): void {
        // Preview surface — robots:noindex set via emit_noindex above; we still
        // give Rank Math an internal-team-friendly title so the WP Admin
        // pages list reads clearly.
        update_post_meta( $page_id, 'rank_math_title',       'Marketing Digital + IA — Variante Light (Preview Interno)' );
        update_post_meta( $page_id, 'rank_math_description', 'Variante light-theme do pillar Marketing Digital para revisão da equipa. Não indexada.' );
    ...
    public static function emit_schema(): void {
        // Preview page — intentionally NO schema emission.
        // Removed from the init() hook list; this stub exists only to avoid
        // breaking any future caller that may still expect the method.
    }
    ...
    public static function render_shortcode(): string {
        ...
        <section class="md-pillar md-pillar--light">
            ...
            <div class="md-final-cta">
                ...
                <p>Os nossos consultores ajudam-te a escolher o curso certo para os teus objetivos e a encontrar as melhores condições de financiamento.</p>

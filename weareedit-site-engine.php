...
define( 'WEAREDIT_SITE_ENGINE_VERSION', '1.5.865' );
...
// scenarios where new code on disk wasn't being executed.
add_action( 'upgrader_process_complete', function ( $upgrader, $hook_extra ) {
    if ( ! function_exists( 'opcache_reset' ) ) return;
    if ( isset( $hook_extra['plugins'] ) && is_array( $hook_extra['plugins'] ) ) {
        foreach ( $hook_extra['plugins'] as $p ) {
            if ( strpos( $p, 'weareedit-site-engine' ) !== false ) { opcache_reset(); return; }
        }
    }
}, 10, 2 );

// Defensive opcache reset on plugin activation. Breaks the opcache-stuck-on-
// old-bytecode loop where the upgrader_process_complete hook above didn't run
// because the OLD plugin code (loaded from stale opcache) didn't have the hook
// registered. Manual deactivate + reactivate cycle now guarantees a fresh
// bytecode load. Use this when card colors / new features aren't taking effect
...
<style id="weareedit-search-rescue">
  ...
  /* The fixed site header (~61px) + the sticky course action bar
     (#info_bar, position:fixed; top:61px; height:60px) together cover the
     top ~121px of the viewport. Without an offset, any in-page jump (a TOC
     link, the Programa / sub-nav menu, or a #anchor) lands the target
     heading UNDER those bars, so the top of the section gets clipped
     (reported on "Opções de pagamento" / the financing box, and applies to
     "Programa do curso" etc. too). Offset every anchor jump so the target
     clears both bars. scroll-padding-top handles the scroll container;
     scroll-margin-top on the targets is the per-element fallback. */
  html:has(body.single-formacao){scroll-padding-top:140px;}
...
require_once WEAREDIT_SITE_ENGINE_PATH . 'includes/class-past-date-interceptor.php';
...
require_once WEAREDIT_SITE_ENGINE_PATH . 'includes/class-links-page.php';
...
require_once WEAREDIT_SITE_ENGINE_PATH . 'includes/class-cf7-antibot.php';
require_once WEAREDIT_SITE_ENGINE_PATH . 'includes/class-gbp-publisher.php';
require_once WEAREDIT_SITE_ENGINE_PATH . 'includes/class-blog-post-og-image.php';
require_once WEAREDIT_SITE_ENGINE_PATH . 'includes/class-blog-cpt-author-support.php';
...
require_once WEAREDIT_SITE_ENGINE_PATH . 'includes/class-search-insights.php';
...
EDIT_Pillar_Tutors::init();
require_once WEAREDIT_SITE_ENGINE_PATH . 'includes/class-pillar-courses.php';
require_once WEAREDIT_SITE_ENGINE_PATH . 'includes/class-alumni-employers.php';
...
require_once WEAREDIT_SITE_ENGINE_PATH . 'includes/class-brevo-bootcamp-inbound.php';
...
require_once WEAREDIT_SITE_ENGINE_PATH . 'includes/class-aai-redesign-preview.php';
require_once WEAREDIT_SITE_ENGINE_PATH . 'includes/class-aai-analytics.php';
require_once WEAREDIT_SITE_ENGINE_PATH . 'includes/class-gama-redesign-preview.php';
require_once WEAREDIT_SITE_ENGINE_PATH . 'includes/class-product-launch.php';
require_once WEAREDIT_SITE_ENGINE_PATH . 'includes/class-link-shortener.php';
require_once WEAREDIT_SITE_ENGINE_PATH . 'includes/class-thankyou-confetti.php';
...
function weareedit_site_engine_init() {
    ...
    EDIT_Brevo_Bootcamp_Inbound::init();
    EDIT_Empresas_Links::init();
    EDIT_Augment_Page::init();
    EDIT_Augment_Feedback::init();
    EDIT_AAI_Redesign_Preview::init();
    EDIT_AAI_Analytics::init();
    EDIT_GAMA_Redesign_Preview::init();
    EDIT_Product_Launch::init();
    EDIT_Link_Shortener::init();
    EDIT_ThankYou_Confetti::init();
